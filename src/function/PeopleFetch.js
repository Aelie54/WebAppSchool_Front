...
export async function fill_grades_store() {
    console.log("fill_grades_store");
    const storeStore = useStudentsStore();
    let response = await fetch("http://127.0.0.1:8000/api/grades", {
            method: "GET",
        })
        .then((r) => r.json())
        .catch();
    storeStore.all_grades = response["hydra:member"];
}

export async function fill_matter_store() {
    console.log("fill_matters_store");
    const storeStore = useStudentsStore();
    let response = await fetch("http://127.0.0.1:8000/api/matters", {
            method: "GET",
        })
        .then((r) => r.json())
        .catch();
    storeStore.all_matters = response["hydra:member"];
}
...
export function empty_grades_store() {
    console.log("empty_grades_store");
    const storeStore = useStudentsStore();
    storeStore.all_grades = [];
}


export function empty_matter_store() {
    console.log("empty_matters_store");
    const storeStore = useStudentsStore();
    storeStore.all_matters = [];
}